...
def index(request):
    ...
    if search_term:
        blog_posts = tuple(filter(lambda blog: search_term in blog.title.lower(
        ) or search_term in blog.content.lower(), blog_posts))
...
def blog(request, id: int):
    ...
    if not request.user.is_authenticated:
        return render(request, 'blog.html', {'blog': blog, 'user_liked': user_liked, 'blog_comments': blog_comments})
...
def post_blog(request):
    ...
    blog = Blog(title=post_title, content=post_body,
                author=user.get_username(), is_public=bool(is_public))
    blog.save()
